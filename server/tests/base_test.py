...
class BaseTest(unittest.TestCase):
    ...
    def add_user(self, **kwargs):
        password = 'qwerty'
        user_info = {'nickname': 'test_name_' + str(self.user_count),
                     'password': password}
        self.user_count += 1
    ...
    def add_arcticle(self, **kwargs):
        article = {'nickname': 'test_name',
                   'preview_content': {'type': 'image', 'data': 'ref'},
                   'tags': ['test_tag_1', 'test_tag_2'],
                   'date': '01.01.2000'
        }
        response = requests.post(self.localhost+'/article', headers={'login': str(kwargs['login']),
