...
который представляет собой статью. Заголовок ``login`` содержит *id* пользователя, который публикует статью.
...
Заголовок ``login`` содержит *id* пользователя, который нажал кнопку лайка.
...
Заголовок ``login`` содержит *id* пользователя, которой написл комментарий. Заголовок ``article-id`` содержит *id*
...
.. code-block:: python
    ...
    :returns: str(json) in format {'login': %STR%}
    """
...
Заголовок ``login`` содержи *id* пользователя, который хочет сменить пароль. Заголовок ``previous-password``
