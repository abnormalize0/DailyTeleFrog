...
def check_password(password, login=None, email=None):
    status = None
    data = None
    if login:
...
def change_password(password, login):
...
def create_comment(article_id, login):
    comment = {'author_id': login,
...
def add_comment(article_id, root_id, comment_text, login):
    status, comment_id, creation_date = create_comment(article_id, login)
    if status.is_error:
        return status, None
...
def vote(db, table_name, id_name, id, login, vote_type):
    ...
    if reverse_vote_id and str(login) in reverse_vote_id.split(config.delimiter):
        status = worker.update_entry(db,
                                     table_name,
                                     id_name,
                                     id,
                                     reverse_vote + '_id',
                                     reverse_vote_id.replace(f'{config.delimiter}{login}{config.delimiter}', ''))
        if status.is_error:
            return status
    ...
    if str(login) not in vote_id.split(config.delimiter):
        status = worker.update_entry(db,
                                     table_name,
                                     id_name,
                                     id,
                                     vote_type + '_id',
                                     vote_id + f'{config.delimiter}{login}{config.delimiter}')
        if status.is_error:
            return status
    else:
        status = worker.update_entry(db,
                                     ...
                                     id,
                                     vote_type + '_id',
                                     vote_id.replace(f'{config.delimiter}{login}{config.delimiter}', ''))
        if status.is_error:
            return status
        change = -1
...
def add_user(info):
    status, login = worker.add_entry(config.db_user.path,
                                       config.user_table_name,
                                       info)
    return status, login

def user_update_info(field_name, field_value, login):
    status = worker.update_entry(config.db_user.path,
                                 config.user_table_name,
                                 config.user_id_name,
                                 login,
...
def user_get_data(login, requested_data):
    status, data = worker.get_entry_data(config.db_user.path,
                                         config.user_table_name,
                                         requested_data,
                                         config.user_id_name,
