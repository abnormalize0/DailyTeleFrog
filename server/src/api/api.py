...
3. Каждый запрос должен принимать на вход login заголовок.
...
def api_article_post():
    status, headers = api_types.parse_structure(request.headers, [api_types.Parameter('login', 'str', True)])
    if status.is_error:
    ...
    status, article_id = backend.post_article(article, headers['login'])
...
def api_article_data_post():
    ...
    if 'like-article' in command:
        status = backend.like_article(headers['article-id'], headers['login'])
        return json.dumps({'status': dict(status)})

    if 'dislike-article' in command:
        status = backend.dislike_article(headers['article-id'], headers['login'])
        return json.dumps({'status': dict(status)})

    if 'like-comment' in command:
        status = backend.like_comment(command['like-comment']['comment_id'], headers['login'])
        return json.dumps({'status': dict(status)})

    if 'dislike-comment' in command:
        status = backend.dislike_comment(command['dislike-comment']['comment_id'], headers['login'])
        return json.dumps({'status': dict(status)})
...
def api_pages_get():
    status, headers = api_types.parse_structure(request.headers,
                                                [api_types.Parameter('login', 'str', True),
...
def api_users_post():
    ...
    status, login = backend.add_user(user_info)
    return json.dumps({'status': dict(status)})
...
def api_users_data_get():
    ...
    status, data = backend.get_user_data(headers['login'], requested_data)
...
def api_users_data_post():
    status, login = api_types.parse_structure(request.headers, [api_types.Parameter('login', 'str', True)])
    if status.is_error:
        return json.dumps({'status': dict(status)})
    login = login['login']
...
def api_login_get():
    ...
    status, login = api_types.parse_structure(request.headers, [api_types.Parameter('email', 'str', False),
                                                                api_types.Parameter('login', 'str', False),])
    if status.is_error:
        return json.dumps({'status': dict(status)})

    if 'login' in login and 'email' in login:
        return json.dumps({'status': dict(request_status.Status(request_status.StatusType.ERROR,
                                          error_type=request_status.ErrorType.ValueError,
    ...
    if 'login' in login:
        status, is_password_correct = backend.login(password, login=login['login'])
    else:
